...
static inline int
__mnl_callback_run(const void *buf, size_t numbytes, unsigned int seq,
		   unsigned int portid, mnl_cb_t cb_data, void *data,
		   mnl_cb_t *cb_ctl_array, unsigned int cb_ctl_array_len,
		   unsigned int flags)
{
	...
	while (mnl_nlmsg_ok(nlh, len)) {
		/* check message source */
		if (!(flags & MNL_CB_F_ANY_PORTID) &&
		    nlh->nlmsg_pid != portid) {
			errno = ESRCH;
			return -1;
		}
		/* perform sequence tracking */
		if (!(flags & MNL_CB_F_ANY_SEQ) && nlh->nlmsg_seq != seq) {
			errno = EPROTO;
			return -1;
		}
...
/**
 * \defgroup deprecated-callback Deprecated callback helpers
 * @{
 */
...
/**
 * mnl_callback_run2 - callback runqueue for netlink messages
 * \param buf buffer that contains the netlink messages
 * \param numbytes number of bytes stored in the buffer
 * \param seq sequence number that we expect to receive
 * \param portid Netlink PortID that we expect to receive
 ...
 */
int
mnl_callback_run2(const void *buf, size_t numbytes, unsigned int seq,
		  unsigned int portid, mnl_cb_t cb_data, void *data,
		  mnl_cb_t *cb_ctl_array, unsigned int cb_ctl_array_len,
		  unsigned int flags)
{
	return __mnl_callback_run(buf, numbytes, seq, portid, cb_data, data,
				  cb_ctl_array, cb_ctl_array_len, flags);
}
EXPORT_SYMBOL(mnl_callback_run2);

/**
 * mnl_callback_run - callback runqueue for netlink messages (simplified version)
 * \param buf buffer that contains the netlink messages
 * \param numbytes number of bytes stored in the buffer
 * \param seq sequence number that we expect to receive
 * \param portid Netlink PortID that we expect to receive
 ...
 */
int
mnl_callback_run(const void *buf, size_t numbytes, unsigned int seq,
		 unsigned int portid, mnl_cb_t cb_data, void *data,
		 unsigned int flags)
{
	return __mnl_callback_run(buf, numbytes, seq, portid, cb_data, data,
...
EXPORT_SYMBOL(mnl_callback_run);
